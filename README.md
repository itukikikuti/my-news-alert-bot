...
- **Web Push 購読 / テスト送信** — 「Push通知を購読する」で Android Chrome からプッシュ通知を受け取り、「Web Push をテスト送信」で動作確認できます
...
6. 管理画面の「Web Push をテスト送信」をタップ（または `curl -X POST https://your-domain.example.com/api/push/send -H "Content-Type: application/json" -d '{"title":"テスト","body":"Web Push 動作確認","url":"/"}'` を実行）
